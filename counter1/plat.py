...
class MultiSourceZoneVisitorCounter(app_callback_class):
    ...
    '''def update_counts(self, camera_id, detected_people):
        ...
        for zone, zone_data in self.data[camera_id]["zones"].items():
            ...
            # Separate people inside and outside the zone based on their center coordinates
            inside_zone_people = {p_id for p_id, x, y in detected_people if self.is_inside_zone(x, y, top_left, bottom_right)}
            
            # Track changes in zone occupancy
            current_zone_occupancy = self.inside_zones[camera_id][zone]
            entered = inside_zone_people - current_zone_occupancy
            exited = current_zone_occupancy - inside_zone_people

            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

            # Process entered people
            if entered:
        ...
        # Update current inside zone tracking
    ...
    def update_counts(self, camera_id, detected_people):
        ...
        for zone, zone_data in self.data[camera_id]["zones"].items():
            ...
            if newly_entered:
                # Only count genuinely new entries
            ...
            if newly_exited:
                # Only count genuinely new exits
                real_new_exits = set()
    ...
    def reset_zone_counts(self, camera_id, zone):
        ...
        #zone_data["history"] = []  # Clear history as well
...
class SafeGStreamerMultiSourceDetectionApp(GStreamerMultiSourceDetectionApp):
    def __init__(self, *args, **kwargs):
        ...
        if threading.current_thread() == threading.main_thread():
            ...
            # Temporarily override signal.signal to avoid crash
...
def handle_set_zone(data):
    ...
    for key in required_keys:
        if key not in data:
            socketio.emit("error", {"message": f"Missing required key: {key}"})
            return
    ...
    try:
        ...
        if (top_left[0] >= bottom_right[0] or top_left[1] >= bottom_right[1]):
            socketio.emit("error", {"message": "Invalid zone coordinates"})
            return
    except (ValueError, TypeError):
        socketio.emit("error", {"message": "Invalid coordinate format"})
        return
...
def start_pipeline():
    global app_instance, video_sources
    data = request.json
    ...
    try:
        app_instance = SafeGStreamerMultiSourceDetectionApp(visitor_counter_callback, user_data, video_sources)
        app_instance.create_pipeline()

        for i in range(len(video_sources)):
            identity_name = f"identity_callback{'' if i == 0 else '_'+str(i)}"
            identity = app_instance.pipeline.get_by_name(identity_name)
            if identity:
        ...
        return jsonify({"success": True, "message": "Pipeline started with sources"}), 200
    except Exception as e:
...
def get_snapshot():
    ...
    # Validate camera_id
    if camera_id not in user_data.data:
        app.logger.error(f"Camera {camera_id} not found in user data")
        return jsonify({"error": f"Camera {camera_id} not found"}), 404
    
    # Check if frame_buffers is initialized and contains the camera
    if not hasattr(frame_buffers, 'get'):
        app.logger.error("frame_buffers is not properly initialized")
        return jsonify({"error": "Snapshot system not ready"}), 500
    
    frame = frame_buffers.get(camera_id)
    if frame is None:
        app.logger.warning(f"No frame available for camera {camera_id}")
    ...
    try:
        ...
        app.logger.error(f"Error encoding snapshot for {camera_id}: {e}")
        return jsonify({"error": "Could not process snapshot"}), 500
